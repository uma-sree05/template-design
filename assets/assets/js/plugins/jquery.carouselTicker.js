  ...
  $.fn.carouselTicker = function (options) {
    ...
    function _eventDragAndDrop() {
      ...
      ticker.$list.on(ticker.eventTypes.mouseup, function (e) {
        ...
        // Allow link clicks only if mouse wasn't moved
        if (
          $target.attr("href") ||
          ($target.parents().attr("href") && ticker.isMousemove)
        ) {
          if (!ticker.isMousemove) {
            // Allow the link click behavior if mouse is not moving
            return;
          }
          e.preventDefault();  // Prevent the link from being clicked while dragging
